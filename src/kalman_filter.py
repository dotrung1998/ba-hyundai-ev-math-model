...
def run_kalman_filter(selected_data, target_forecast_months):
    """
    Kalman-Filter mit Neusser (2016) Notation und Visualisierung der realistischen Abwertungsrate.
    """
    # Extrahiere Daten
    ...
    # --- ANPASSUNG START ---
    # Berechne eine zeitlich veränderliche "realistische" Abwertungsrate als Stufenfunktion
    ...
    # Die alte, konstante `monthly_rate`-Berechnung wurde entfernt.
    # --- ANPASSUNG ENDE ---

    # Erstelle die realistische Wertkurve für die Visualisierung
    true_rv_curve_plotting = []
    for t in time_vector:
        if t <= 12:
            rv = initial_rv * (1 - 0.25 * (t / 12))
        elif t <= 24:
            # Berechnung basierend auf dem Restwert nach Jahr 1
            rv = value_after_year1 * (1 - 0.18 * ((t - 12) / 12))
        else:
            # Berechnung basierend auf dem Restwert nach Jahr 2
    ...
    # Systemmatrizen nach Neusser (2016)
    F = np.array([[1, -dt], [0, 1]])
    A = np.array([[0]])
    G = np.array([[1, 0]])

    # Rauschparameter für 2025 EV-Markt
    Q = np.array([[150, 0], [0, 15]])
    R = np.array([[750 ** 2]])

    # Simuliere verrauschte Beobachtungen
    ...
    # Initialisierung
    # Verwende die Rate am Anfang des Zeitraums für die Initialisierung
